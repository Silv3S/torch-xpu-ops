...
void linalg_eigh_kernel_fallback(
  ...
  linalg_eigh_stub(at::kCPU, eigenvalues_cpu, eigenvectors_cpu, infos_cpu, upper, compute_eigenvectors);
...
void linalg_eigh_kernel_xpu(
    ...
    bool compute_eigenvectors) {
#if defined(USE_ONEMKL_XPU)
  native::xpu::eigh_mkl(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
#else
  linalg_eigh_kernel_fallback(eigenvalues, eigenvectors, infos, upper, compute_eigenvectors);
#endif // USE_ONEMKL_XPU
}
