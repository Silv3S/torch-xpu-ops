...
void nonzero_kernel_impl(
    ...
    int64_t* divisor,
    int64_t* sizes) {
  sycl::nd_item<1> it = syclext::this_work_item::get_nd_item<1>();
  auto global_id = it.get_global_linear_id();
...
void nonzero_template(const Tensor& self_, Tensor& tensor) {
  ...
  if (N > 0) {
    ...
    if (num_nonzeros > 0 && num_dim > 0) {
      ...
      // auto devIndex = at::xpu::getDeviceIndexOfCurrentQueue();
      // const auto wg_sz = std::min(syclMaxWorkGroupSize<nonzero_kernel_impl>(devIndex), N);
