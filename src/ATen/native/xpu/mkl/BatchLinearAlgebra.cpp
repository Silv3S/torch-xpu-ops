...
void svd_mkl_impl(
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    ...
    // ODWROTNIE!!!!
    scalar_t* vt_ptr = compute_uv ? U.select(0, i).data_ptr<scalar_t>() : nullptr;
    scalar_t* u_ptr = compute_uv ? Vh.select(0, i).data_ptr<scalar_t>() : nullptr;

    
    try {
      oneapi::mkl::lapack::gesvd(
          queue, jobz, jobz, n, m, a_ptr, lda,
          s_ptr, u_ptr, ldu, vt_ptr, ldvt,
          buffer, bufsize);
    } catch (const oneapi::mkl::lapack::exception& e) {
      info_data[i] = e.info();
    }
...
void eigh_mkl_impl(
  ...
  TORCH_CHECK(eigenvectors.device().is_xpu(), "eigenvectors must be an XPU tensor");
  ...
  auto jobz = compute_eigenvectors ? oneapi::mkl::job::vec : oneapi::mkl::job::novec;
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    ...
    try {
      oneapi::mkl::lapack::syevd(
          queue, jobz, uplo, n, a_ptr, lda,
          w_ptr, buffer, bufsize);
    } catch (const oneapi::mkl::lapack::exception& e) {
      infos_data[i] = e.info();
    }
  }
...
void eigh_mkl(
    ...
    bool compute_eigenvectors) {
  AT_DISPATCH_FLOATING_TYPES(
      eigenvectors.scalar_type(), "eigh_mkl_xpu", [&] {
